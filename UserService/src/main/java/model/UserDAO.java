...
public class UserDAO implements DAO<User> {
    ...
    public Optional<User> get(String key) {
        User user = new User();
        try (PreparedStatement stmt = con.prepareStatement("SELECT * FROM USER_DKE WHERE nickname = ?")) {
            stmt.setString(1, key);
            System.out.println(key);
            ResultSet result = stmt.executeQuery();
            if(!result.next()) return Optional.empty();
            user = new User(result.getString("nickname"), result.getString("first_name"), result.getString("last_name"), result.getString("email"), result.getString("password"), result.getBlob("portrait"), result.getDate("birthday"), null);
        } catch (Exception e) {
    ...
    public List<User> getAll() {
        String SELECT = "SELECT nickname, first_name, last_name, email, password, PORTRAIT, BIRTHDAY, SEX FROM user_dke;";
    ...
    public void update(User user) {
        try (PreparedStatement preparedStatement = con.prepareStatement("UPDATE USER_DKE SET first_name = ?, last_name = ?, email = ?, password = ?, PORTRAIT = ?, BIRTHDAY = ?, SEX = ? WHERE nickname = ?")) {
            if(user.getNickname() == null) throw new NullPointerException("Nickname can't be NULL");
            else preparedStatement.setString(8, user.getNickname());
            if(user.getFirst_name() == null) preparedStatement.setNull(1, Types.VARCHAR);
            else preparedStatement.setString(1, user.getFirst_name());;
            if(user.getLast_name() == null) preparedStatement.setNull(2, Types.VARCHAR);
            else preparedStatement.setString(2, user.getLast_name());
            if(user.getEmail() == null) throw new NullPointerException("Email can't be NULL");
            else preparedStatement.setString(3, user.getEmail());
            if(user.getPassword() == null) throw new NullPointerException("Password can't be NULL");
            else preparedStatement.setString(4, user.getPassword());
            if(user.getPortrait() == null) preparedStatement.setNull(5, Types.BLOB);
            else preparedStatement.setBlob(5, user.getPortrait());
            if(user.getBirthdate() == null) preparedStatement.setNull(6, Types.DATE);
            else preparedStatement.setDate(6, user.getBirthdate());
            if(user.getSex() == null) preparedStatement.setNull(7, Types.CHAR);
            else preparedStatement.setObject(7, user.getSex());
            preparedStatement.executeQuery();
            con.commit();
        } catch (Exception sql) {
            System.out.println("SQL Exception: During update transaction");
            sql.printStackTrace();
            try {
                con.rollback();
            } catch (SQLException e) {
    ...
    public void delete(String key) {
        try (PreparedStatement stmt = con.prepareStatement("DELETE FROM user_dke WHERE nickname = ?")) {
            stmt.setString(1, key);
            stmt.executeQuery();
            con.commit();
